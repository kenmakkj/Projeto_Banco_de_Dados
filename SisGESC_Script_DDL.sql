...
-- [C2] GOV — adicionados data_criacao e ultima_atualizacao (rastreabilidade)
...
-- [C1] 1FN — catálogo atômico de áreas de atuação
...
-- [C1] 1FN — tabela associativa N:N professor <-> área
...
-- tb_notas e tb_aulas dependem de tb_professores — ficam após sua criação
...
-- [C3] GOV — adicionados data_criacao e ultima_atualizacao (rastreabilidade + RN13)
...
-- RN12 — PK composta impede a mesma verba duas vezes no mesmo holerite
...
-- [C4] N:N — tb_descontos_bolsas agora é catálogo puro (sem fk_contrato)
...
-- [C4] N:N — entidade associativa contrato <-> desconto
--      RN14 aplicada aqui via CHECK.
...
-- RH (cargo atual — data_fim IS NULL)
SELECT
  p.pk_cpf,
  p.primeiro_nome,
  p.sobrenome,
...
-- [C5] FINANCEIRO — corrigido: me.valor -> me.valor_liquido
--                              JOIN status via me.fk_status (não pa.fk_status)
SELECT
  pe.pk_cpf,
  pe.primeiro_nome,
  pe.sobrenome,
