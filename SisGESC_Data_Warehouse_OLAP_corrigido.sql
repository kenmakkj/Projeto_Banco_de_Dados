...
SELECT
    ...
    ROUND(100.0 * SUM(fr.valor_pago) / NULLIF(SUM(fr.valor_mensalidade_liquido),0), 2) AS taxa_recebimento,
...
SELECT
    ...
    ROUND(100.0 * SUM(CASE WHEN fd.eh_aprovado  THEN 1 ELSE 0 END) / NULLIF(COUNT(*),0), 2)     AS taxa_aprovacao,
    ROUND(100.0 * SUM(CASE WHEN fd.eh_reprovado THEN 1 ELSE 0 END) / NULLIF(COUNT(*),0), 2)     AS taxa_reprovacao
...
DROP PROCEDURE IF EXISTS sp_etl_carregar_tempo $$
CREATE PROCEDURE sp_etl_carregar_tempo()
BEGIN
    DECLARE v_data     DATE;
...
DROP PROCEDURE IF EXISTS sp_etl_carregar_unidades $$
CREATE PROCEDURE sp_etl_carregar_unidades()
BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
...
DROP PROCEDURE IF EXISTS sp_etl_carregar_alunos $$
CREATE PROCEDURE sp_etl_carregar_alunos()
BEGIN
    ...
    -- [FIX-04] tb_contrato_desconto → tb_descontos_bolsas (nome correto no Dicionário, p.15)
    LEFT JOIN (
        SELECT db.fk_contrato, MAX(db.tipo_bolsa) AS tipo_bolsa
        FROM sisgesc.tb_descontos_bolsas db
        GROUP BY db.fk_contrato
...
DROP PROCEDURE IF EXISTS sp_etl_carregar_cursos $$
CREATE PROCEDURE sp_etl_carregar_cursos()
BEGIN
    DECLARE v_registros INT DEFAULT 0;
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
...
DROP PROCEDURE IF EXISTS sp_etl_carregar_professores $$
CREATE PROCEDURE sp_etl_carregar_professores()
BEGIN
    DECLARE v_registros INT DEFAULT 0;
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
...
DROP PROCEDURE IF EXISTS sp_etl_carregar_disciplinas $$
CREATE PROCEDURE sp_etl_carregar_disciplinas()
BEGIN
    DECLARE v_registros INT DEFAULT 0;
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
    ...
    SELECT
        ...
        d.nome,
        d.carga_horaria,
        CEIL(d.carga_horaria / 15),
        TRUE,
        TRUE
    FROM sisgesc.tb_disciplinas d;
...
DROP PROCEDURE IF EXISTS sp_etl_carregar_fato_receita $$
CREATE PROCEDURE sp_etl_carregar_fato_receita()
BEGIN
    DECLARE v_registros INT DEFAULT 0;
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
    ...
    SELECT
        ...
        m.pk_mensalidade,
        m.valor_liquido,
        -- [FIX-05] sisgesc.tb_contrato_desconto → sisgesc.tb_descontos_bolsas
        COALESCE((
            SELECT SUM(
                COALESCE(db.valor_fixo_desconto, 0)
                + (m.valor_liquido * COALESCE(db.percentual_desconto, 0) / 100.0)
            )
            FROM sisgesc.tb_descontos_bolsas db
            WHERE db.fk_contrato = ce.pk_contrato
        ), 0),
        m.valor_multa,
        m.valor_juros,
...
DROP PROCEDURE IF EXISTS sp_etl_carregar_fato_desempenho $$
CREATE PROCEDURE sp_etl_carregar_fato_desempenho()
BEGIN
    DECLARE v_registros INT DEFAULT 0;
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
    ...
    SELECT
        ...
        m.pk_matricula,
        rm.media_final,
        ROUND(100.0 * (1 - (COALESCE(rm.total_faltas, 0) / NULLIF(d.carga_horaria, 0))), 0),
    ...
    JOIN sisgesc.tb_aulas au        ON au.fk_turma      = m.fk_turma
...
DROP PROCEDURE IF EXISTS sp_etl_carregar_fato_folha $$
CREATE PROCEDURE sp_etl_carregar_fato_folha()
BEGIN
    DECLARE v_registros INT DEFAULT 0;
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
    ...
    SELECT
        dp.sk_professor,
        CAST(DATE_FORMAT(DATE(CONCAT(fp.ano, '-', LPAD(fp.mes, 2, '0'), '-01')), '%Y%m%d') AS UNSIGNED),
        fp.pk_folha,
        fp.mes,
        fp.ano,
        fp.salario_bruto,
...
DROP PROCEDURE IF EXISTS sp_etl_carga_completa_dw $$
CREATE PROCEDURE sp_etl_carga_completa_dw()
BEGIN
